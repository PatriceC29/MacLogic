...
class Backup:
    ...
    def setup(self):
        ...
        while True:
            q_name = raw_input(questions["name"])
            fname = "{}/{}.dmg".format(self.target, q_name)
            print fname
    ...
    def rsync(self, source, rtarget):
        ...
        command = "./bin/{} -XEva --inplace {} {}".format(arch, source, rtarget)
        print("Copie de {} vers {}...".format(source, rtarget))
        process = subprocess.Popen(shlex.split(command),
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE,
                                   universal_newlines=True)
        stdout, stderr = process.communicate()
        self.logger.info("RSYNC:{}\n{}".format(source, stdout))
        if stderr:
           self.logger.error("RSYNC:{}\n{}".format(source, stderr))
    ...
    def get_size_b(self, source):
        arch = "irsync" if self.arch() == 1 else "arsync"
        args = '-XEvan {} /tmp 2>/dev/null | grep -E "total size is [0-9,]+" -o | tr -d "," | grep -o -E "[0-9]+"'.format(source)
        command = "./bin/{} {}".format(arch, args)
        ...
        print("{}: {}o".format(source, self.human_readable(size)))
